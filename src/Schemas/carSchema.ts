...
import { z } from "zod";
import {   carAvailabilityArray,
    carAvailableAreaArray,
    carCategoryArray,
    carTypeArray, } from "../Const/car.const";
...
// update section 
const updateTCarCategory = z.enum(carCategoryArray as [string, ...string[]], {
  errorMap: () => ({ message: "Invalid car category" }),
}).optional();
const updateTCarType = z.enum(carTypeArray as [string, ...string[]], {
  errorMap: () => ({ message: "Invalid car type" }),
}).optional();

const updateTAvailableArea = z.enum(carAvailableAreaArray as [string, ...string[]], {
  errorMap: () => ({ message: "Invalid available area" }),
}).optional();
...
const updateCarZodSchema = z.object({
    ...
    licensePlate: z.string().min(1, { message: "License plate is required" }).optional(),
    color: z.string().min(1, { message: "Color is required" }).optional(),
    mileage: z
      .number()
      .min(0, { message: "Mileage must be a non-negative number" }).optional(),
    rentalPricePerDay: z.number().min(0, {
      message: "Rental price per day must be a non-negative number",
    }).optional(),
    advance: z
      .number()
      .min(0, { message: "Advance must be a non-negative number" }).optional(),
    availability: z
      .enum(carAvailabilityArray as [string, ...string[]], {
        errorMap: () => ({ message: "Invalid availability status" }),
      })
      .optional(),
    availableAreas: z.array(updateTAvailableArea, {
      errorMap: () => ({ message: "Each area must be a valid available area" }),
    }).optional(),
    numberOfDoors: z
      .number()
      .min(1, { message: "Number of doors must be at least 1" }).optional(),
    seatingCapacity: z
      .number()
      .min(1, { message: "Seating capacity must be at least 1" }).optional(),
    features: z.array(z.string().min(1), {
      errorMap: () => ({ message: "Each feature must be a non-empty string" }),
    }).optional(),
    safetyFeatures: z.array(z.string().min(1), {
